...
/*
one of the documents contains a list of left/right instructions, and the rest of the documents seem to describe some kind of network of labeled nodes.
...
const parseNodes = (content: string) => {
  const [instructionsStr, nodesStr] = content.split('\n\n');

  const instructions = instructionsStr.trim().split('') as Direction[];

  const nodeLines = nodesStr.trim().split('\n').filter(Boolean);

  const leftMap = new Map<string, string>();
  const rightMap = new Map<string, string>();

  for (const line of nodeLines) {
    const match = /^(\w+)\s*=\s*\((\w+),\s*(\w+)\)$/.exec(line.trim());
    if (!match) continue;
