...
function buildInitialTableau(model) {
  ...
  const tableau = Array.from({ length: numConstraints + 1 }, () => Array.from({ length: totalVars + 1 }).fill(0));
  const basis = Array.from({ length: numConstraints }).fill(-1);

  let slackOffset = numOriginalVars;
  let artificialOffset = numOriginalVars + slackCount;
  const slackIndex = Array.from({ length: numConstraints }).fill(null);
  const artificialIndex = Array.from({ length: numConstraints }).fill(null);
...
export default Puzzle.new({
  ...
  //   }),
  easy(content) {
    const machines = Str.lines(content).map((line) => {
      const strs = line.split(' ').map((s) => s.substring(1, s.length - 1));
      const diagramStr = strs.shift()!;
      const diagram = diagramStr.split('').map((c) => c === '#');
      const joltage = strs.pop()!.split(',').map((n) => +n);
    ...
    return sumBy(machines, ({ diagram, buttons }) => {
      ...
      const stateKey = (state: boolean[]) => state.join(',');

      const isExpectedState = (state: boolean[], expected: boolean[]) => {
  ...
  hard(content) {
    const machines = parseMachineInstructions(content);

    return sumBy(machines, (machine) => {
      return solveMachineForJoltage(machine);
    });
  },
});
...
type State = typeof OFF | typeof ON;
type Char = '.' | '#';
...
function parseMachineInstructions(input: string) {
  const lines = Str.lines(input);

  function parseMachine(line) {
    const lightDiagram: { char: Char; targetState: State; currentState: State }[] = [];
    const expectedDiagram = [];
    const expectedJoltage = [];
    const tokenParser = {
      '[': parseLightDiagram,
      '(': parseWiringSchematic,
      '{': parseexpectedJoltage,
    };

    function parseLightDiagram(token: string) {
    ...
    function parseWiringSchematic(token: string) {
      const indexes = token.slice(1, -1).split(',').map(Number);

      const buttons = indexes;

      expectedDiagram.push({ buttons });
    }

    function parseexpectedJoltage(token: string) {
      const values = token.slice(1, -1).split(',').filter((value) => value.length > 0).map(Number);
      expectedJoltage.push(...values);
    }

    const tokens = line.split(' ').map((token) => tokenParser[token.charAt(0)](token));
...
function solveMachineForJoltage(machine) {
  const targets = machine.expectedJoltage;

  const buttons = machine.expectedDiagram
    .map((schematic) => {
      const effect = Array.from(targets.length).fill(0);
  ...
  const coverage = Array.from(targets.length).fill(0);
  for (const button of buttons) {
  ...
  for (let i = 0; i < targets.length; ++i) {
    const coeffs = buttons.map((button) => button[i] || 0);

    constraints.push({ relation: '=', rhs: targets[i], coeffs });
  }

  for (let i = 0; i < buttons.length; ++i) {
    const coeffs = Array(buttons.length).fill(0);

    coeffs[i] = 1;

    constraints.push({ relation: '>=', rhs: 0, coeffs });
  }

  return sum(
    solveIntegerProgram({
      numVars: buttons.length,
      objective: { sense: 'min', coeffs: Array.from({ length: buttons.length }).fill(1) },
      constraints,
    }).solution.map(Math.round),
  );
}
